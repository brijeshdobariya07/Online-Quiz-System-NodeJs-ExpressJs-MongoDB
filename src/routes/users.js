...
router.get('/tests/:name/delete/:testName/:testCode',async(req,res)=>{
    ...
    const deleted = await Exam.deleteOne({testName:testName,testCode:testCode});
...
router.post('/custom/:name',async(req,res)=>{
    ...
    const {name,email,testName,testCode,question,answer_a,answer_b,answer_c,answer_d,correct_answer} = req.body;
    let newData = [];
    let queObj = {};
    let ansObj = {};
