...
router.post('/dashboard',async(req,res)=>{
    ...
    const url = "https://quizapi.io/api/v1/questions?apiKey=PgCAQzEcs1eGMj5R1Dyt7Qk7bPtI3Rl7QmAVc3iC&category=" + category + "&difficulty=" + difficulty +"&limit=" + limit;
    ...
    const data = await axios.get(url).then(async(res)=>{
        ...
        const newExam = {
            name,email,testName,testCode,newData
        }
...
router.post('/code',async(req,res)=>{
    ...
    const examCode = await Exam.findOne({testCode:testCode},{_id:0}).select({testCode:1})
...
router.post('/quiz',async(req,res)=>{
    ...
    const marks = score;
    const studentData = {
        name,rollno,testName,testCode,marks
    }
